...
public class TextureAsset extends TextureRegion {
	...
	public TextureAsset cut( int x, int y ) {
		...
		for(int i=0; i<x*y; i++) {
			...
			tile.setRegion( this.getRegionX() +i%x *w,
							this.getRegionY() +i/x *h, w, h);
			tile.update();
			tiles[i%x][i/x] = tile;
		}
	...
	public TextureAsset getTile( int x, int y ) {
		if(tiles == null || x<0 || y<0 || x>=tiles.length || y>=tiles[0].length) return null;
		return tiles[x][y];
	}
	public TextureAsset getTile( int id ) {
		if(id<0 || id>= tiles.length*tiles[0].length) return null;
